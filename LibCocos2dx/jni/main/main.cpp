...
extern "C"{
    ...
    //礼物控制
    jint Java_org_cocos2dx_lib_util_Cocos2dxGift_play2(JNIEnv *env, jobject thiz,jstring aniName,jstring imagePath,jstring plistPath,jstring exportJsonPath,jfloat scale,jint x,jint y)
     {
        const char * aniNameStr = env->GetStringUTFChars(aniName, 0);
        const char * imagePathStr = env->GetStringUTFChars(imagePath, 0);
        const char * plistPathStr = env->GetStringUTFChars(plistPath, 0);
        const char * exportJsonPathStr = env->GetStringUTFChars(exportJsonPath, 0);
        int result = pAppDelegate->play(aniNameStr,imagePathStr,plistPathStr,exportJsonPathStr,scale,x,y);
        return (jint)result;
    }
